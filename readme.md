...
- **Natural language decisions** — No menus. Type what you do, or pick from AI-generated choices. Or both.
...
### Strategic Controls (always available)

**Pace** — Set in the status panel. Affects miles per day and health drain.
...
**Rations** — Set in the status panel. Affects food consumption and health.
...
- 🦌 **Hunt** — Stop to hunt. Outcome depends on zone and weather. Buffalo on the plains, deer in the mountains, almost nothing in the desert.
- 🛌 **Rest** — Make camp. Health recovers. Distance stops.
- 🏪 **Trade at Fort** — Appears when within 40 miles of Fort Kearney, Fort Laramie, or Fort Hall. Buy food, get trail information, repair equipment.

### AI-Generated Choices

After every narration, the AI presents 3–4 concrete options specific to the current situation — plus "Something else..." to type freely. One choice is safe and slow. One is bold and risky. One is creative. Picking wisely matters.

---

## Event System

The game engine rolls a zone-appropriate event every turn. The AI weaves it into the scene and closes with it as the next decision bearing down on the party.

| Zone | Miles | Primary Threats |
|------|-------|----------------|
| Early Plains | 0–316 | Cholera, Kansas River crossing, Pawnee tribute, scorpions, mud |
...
| Global | Any | Illness, lame ox, broken axle, wheel failure, strangers, food spoilage, storms |
...
- ⚡ **Pace system** — 5 settings from Grueling to Resting, each with real health tradeoffs
...
- 🦌 **Hunting** — Zone and weather-aware outcomes. Narrated vividly win or lose.
- 🏪 **Fort trading** — Fort Kearney, Fort Laramie, Fort Hall. Buy supplies, get trail intel.
- 🏹 **Native encounter system** with hidden disposition — friendly, neutral, or hostile
- 🎨 **AI scene illustrations** — every turn generates a unique frontier image via Pollinations.AI
- 🎬 **Ken Burns effect** — 4 randomized pan/zoom animations bring images to life
- 💾 **localStorage save** — auto-saves every turn, Continue Journey on return
- 📓 **Trail Journal** — writes a readable `.md` journal to your device; AI reads it as long-term memory (Chrome/Edge)
- 🃏 **AI-generated choices** — 3–4 concrete options per turn plus free-text input
- 📍 11 historical landmarks from Alcove Spring to Oregon City
- ☀️ Dynamic weather system affecting travel and consumption
...
Before starting, click "Connect Trail Journal" and choose a folder. The game creates `oregon-trail-journal.md` there and writes every scene as readable markdown. On long games the AI reads the full journal back as long-term memory.
...
Handles resource math, pace/rations calculations, zone detection, event rolling, fort proximity detection, location tracking, and win/death conditions. The AI never touches these calculations.

**Strategic Systems**
Pace and rations are set by the player and applied every turn in `dailyTick()`. Pace bounds the AI's `distance_change`. Rations multiply food consumption. Health drifts based on both settings simultaneously, creating compound pressure on long difficult stretches.

**Event System**
Each turn, `rollEvent()` selects a zone-weighted, repeat-filtered event with 12% quiet-day probability. Events are injected with historical stakes and, for Native encounters, a hidden disposition. The AI evaluates the player's response against the actual situation.

**Action System**
Quick actions (Hunt, Rest, Continue, Trade) inject specific context into the prompt. The AI knows it's a hunting attempt vs. a rest day vs. a fort visit and responds accordingly — different narration, different resource outcomes, different dialogue.

**AI Layer (Groq API — Llama 3.3 70B)**
One call per turn. Full game state, pace, rations, active event, action type, and journal context all injected. Returns strict JSON with narration, dialogue, 3–4 player choices, state effects, weather, and image prompt.

**Memory System**
Three layers: last 16 conversation turns (short-term), last 8 event summaries (mid-term), full Trail Journal from filesystem (long-term, optional).

**Image Layer (Pollinations.AI — Flux)**
AI crafts an `image_prompt` from the scene. Engine appends frontier style guidance, random seed, and fetches asynchronously. Four randomized Ken Burns animations applied on load.
...
| AI Images | Pollinations.AI (Flux) | Completely free, no key, no account |
